...
struct Cli {
    #[arg(
        long = "firehose-enable",
        default_value = "false",
        env = "FIREHOSE_ENABLE"
    )]
    firehose_enable: bool,
    #[arg(
        long,
        value_name = "DIR",
...
impl Config {
    pub fn parse() -> Self {
        ...
        std::env::set_var("FIREHOSE_ENABLE", cli.firehose_enable.to_string());

        Config {
            firehose_enable: cli.firehose_enable,
