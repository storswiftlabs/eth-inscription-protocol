    ...
    GatewayClient: GatewayApi + Clone + Send + 'static,
{
    ...
    'outer: loop {
        ...
        fire_print_block_data(block.as_ref());
...
fn fire_print_block_data(block: &Block) {
    if std::env::var("FIREHOSE_ENABLE").unwrap_or(String::from("false")) != "true" {
        return;
    };
    ...
    use crate::proto::{
        block_data::{
            transaction_receipt::{
                execution_resources::BuiltinInstanceCounter as PbBuiltinInstanceCounter,
                Event as PbEvent, ExecutionResources as PbExecutionResources,
                L1ToL2Message as PbL1ToL2Message, L2ToL1Message as PbL2ToL1Message,
            },
            TransactionReceipt as PbTransactionReceipt,
        },
        transaction::Transaction as PbEnumTransaction,
        Transaction as PbTransaction, TransactionType as PbTransactionType,
    };
    use starknet_gateway_types::reply::transaction::{
        DeclareTransaction, EntryPointType, InvokeTransaction, Transaction,
    };

    let pb_transaction_receipts = block
    ...
    let pb_transactions = block
        .transactions
        .iter()
        .map(|tx| match tx {
            Transaction::Declare(tx) => match tx {
                DeclareTransaction::V0(tx) => PbTransaction {
                    transaction: Some(PbEnumTransaction::DeclareTransaction(
                ...
                DeclareTransaction::V1(tx) => PbTransaction {
                    transaction: Some(PbEnumTransaction::DeclareTransaction(
                        crate::proto::DeclareTransaction {
                            r#type: PbTransactionType::Declare.into(),
                ...
                DeclareTransaction::V2(tx) => PbTransaction {
                    transaction: Some(PbEnumTransaction::DeclareTransaction(
                        crate::proto::DeclareTransaction {
                            r#type: PbTransactionType::Declare.into(),
            ...
            Transaction::Deploy(tx) => PbTransaction {
            ...
            Transaction::DeployAccount(tx) => PbTransaction {
            ...
            Transaction::Invoke(tx) => PbTransaction {
                transaction: Some(PbEnumTransaction::InvokeTransaction(match tx {
            ...
            Transaction::L1Handler(tx) => PbTransaction {
    ...
    let pb_block = crate::proto::BlockData {
        ...
        starknet_version: block.starknet_version.clone().take_inner(),
    };

    let block_bytes = pb_block.encode_to_vec();
    let block_base64 = base64::encode(block_bytes);

    println!("FIRE BLOCK_DATA {} {}", block.block_number, block_base64);
}
