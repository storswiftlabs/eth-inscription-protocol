...
impl Transaction {
        ...
        match self.transaction_type {
            ...
            Some(x) if x == U64::from(1) => {
                ...
                if let Some(chain_id) = self.chain_id {
                    rlp.append(&normalize_v(self.v.unwrap_or_default().as_u64(), U64::from(chain_id.as_u64())));
                }
            }
            // EIP-1559 (0x02)
    ...
    pub fn recover_from(&self) -> Result<Address, SignatureError> {
        let signature = Signature { r: self.r.unwrap_or_default(), s: self.s.unwrap_or_default(), v: self.v.unwrap_or_default().as_u64() };
...
impl Decodable for Transaction {
    fn decode(rlp: &rlp::Rlp) -> Result<Self, DecoderError> {
        ...
        if rlp.is_list() {
            ...
            txn.v = Some((odd_y_parity as u8).into());
...
mod tests {
    ...
    fn rlp_legacy_tx() {
        let tx = Transaction {
            ...
            v: Some(U64::from(0x25)),
            r: Some(U256::from_str_radix("c81e70f9e49e0d3b854720143e86d172fecc9e76ef8a8666f2fdc017017c5141", 16).unwrap()),
            s: Some(U256::from_str_radix("1dd3410180f6a6ca3e25ad3058789cd0df3321ed76b5b4dbe0a2bb2dc28ae274", 16).unwrap()),
            chain_id: Some(U256::from(1)),
            access_list: None,
