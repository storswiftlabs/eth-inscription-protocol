...
function main() {
  ...
  echo "streamingfast/firehose-starknet/proto revision: $(GIT_DIR=$ROOT/.git git log -n 1 --pretty=format:%h -- proto)" >>./last_generate.txt
}
...
function generate() {
  ...
  for file in "$@"; do
    protoc -I$PROTO -I$PROTO_STARKNET \
