...
import (
	...
	"github.com/streamingfast/firehose-starknet/codec"
	"github.com/streamingfast/firehose-starknet/nodemanager"
...
var nodeLogger, nodeTracer = logging.PackageLogger("node", "github.com/streamingfast/firehose-starknet/node")
var nodeStarknetChainLogger, _ = logging.PackageLogger("node.starknet", "github.com/streamingfast/firehose-starknet/node/starknet", DefaultLevelInfo)

var readerLogger, readerTracer = logging.PackageLogger("reader", "github.com/streamingfast/firehose-starknet/reader")
var readerStarknetChainLogger, _ = logging.PackageLogger("reader.starknet", "github.com/streamingfast/firehose-starknet/reader/starknet", DefaultLevelInfo)
...
func registerCommonNodeFlags(cmd *cobra.Command, flagPrefix string, managerAPIAddr string) {
	...
	`, flagPrefix+"path"))
	cmd.Flags().String(flagPrefix+"manager-api-addr", managerAPIAddr, "Starknet node manager API address")
...
func registerNode(kind string, extraFlagRegistration func(cmd *cobra.Command) error, managerAPIaddr string) {
	...
	launcher.RegisterApp(rootLog, &launcher.AppDef{
		ID:          app,
		Title:       fmt.Sprintf("Starknet Node (%s)", kind),
		Description: fmt.Sprintf("Starknet %s node with built-in operational manager", kind),
...
func nodeFactoryFunc(flagPrefix, kind string) func(*launcher.Runtime) (launcher.App, error) {
	return func(runtime *launcher.Runtime) (launcher.App, error) {
		...
		case "node":
			...
			supervisedProcessLogger = nodeStarknetChainLogger
		case "reader":
			appLogger = readerLogger
			appTracer = readerTracer
			supervisedProcessLogger = readerStarknetChainLogger
		default:
