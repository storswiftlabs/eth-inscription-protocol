...
func printBlocksE(cmd *cobra.Command, args []string) error {
	...
	for {
		...
		starknetBlock := block.ToProtocol().(*pbstarknet.BlockData)

		fmt.Printf("Block #%d (%s) (prev: %s): %d transactions\n",
			block.Num(),
			block.ID()[0:7],
			block.PreviousID()[0:7],
			len(starknetBlock.Transactions),
		)
		if printTransactions {
			fmt.Println("- Transactions: ")
			for _, t := range starknetBlock.Transactions {
				switch t := t.GetTransaction().(type) {
				case *pbstarknet.Transaction_DeclareTransaction:
					fmt.Println("  * ", t.DeclareTransaction.TransactionHash)
				case *pbstarknet.Transaction_DeployTransaction:
					fmt.Println("  * ", t.DeployTransaction.TransactionHash)
				case *pbstarknet.Transaction_DeployAccountTransaction:
					fmt.Println("  * ", t.DeployAccountTransaction.TransactionHash)
				case *pbstarknet.Transaction_InvokeTransaction:
					fmt.Println("  * ", t.InvokeTransaction.TransactionHash)
				case *pbstarknet.Transaction_L1HandlerTransaction:
					fmt.Println("  * ", t.L1HandlerTransaction.TransactionHash)
				default:
				}
	...
	for {
		...
		if printTransactions {
			...
			for _, t := range starknetBlock.Transactions {
				switch t := t.GetTransaction().(type) {
				case *pbstarknet.Transaction_DeclareTransaction:
					fmt.Printf("  * %s\n", t.DeclareTransaction.TransactionHash)
				case *pbstarknet.Transaction_DeployTransaction:
					fmt.Printf("  * %s\n", t.DeployTransaction.TransactionHash)
				case *pbstarknet.Transaction_DeployAccountTransaction:
					fmt.Printf("  * %s\n", t.DeployAccountTransaction.TransactionHash)
				case *pbstarknet.Transaction_InvokeTransaction:
					fmt.Printf("  * %s\n", t.InvokeTransaction.TransactionHash)
				case *pbstarknet.Transaction_L1HandlerTransaction:
					fmt.Printf("  * %s\n", t.L1HandlerTransaction.TransactionHash)
				default:
				}
			}
		}
...
func printBlock(block *bstream.Block) error {
	nativeBlock := block.ToProtocol().(*pbstarknet.BlockData)
